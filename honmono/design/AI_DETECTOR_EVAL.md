> **2026-09-17 訂正（本文中の当時の説明より優先）**
> この文書は過去の試験記録です。「全源商用可」「再配布の問題なし」という当時の評価は、画像の利用条件と取得許諾を混同しており、撤回します。取得方法に関する契約上の論点と、情報解析・モデル配布の評価を分けます。個別許諾書・全面再学習を一律の公開条件にはしません。詳細は同じフォルダーの DATA_RIGHTS_REVIEW_20260917.md に記録します。
> 545枚はしきい値選択に参照しています。顔615枚は選択用devと同じ元画像群から切り出した追加診断であり、完全に独立した評価ではありません。
> v3.1のAUCは0.932、先行v3のAUCは0.952です。スコアは個別画像の真偽確率ではありません。
> 最新の説明とブラウザ検証は [実測レポート](../report/) と [権利確認状況](DATA_RIGHTS_REVIEW_20260917.md) を参照してください。
...
- 現状の公開状態(2026-08-19 更新): **自前学習モデル HONMONO v3.1(商用利用可のデータのみ)を同梱して有効化**(下の追記2)。借り物モデルは同梱していない
...
|---|---|---|---|---|---|---|---|
| **v3.1(採用・しきい値0.8)** | 0.932 | **1.8%** | **70.0%** | **62.6%** | 0.932 | **5.0%** | **全源が商用可** |
| v3(同日先行版・しきい値0.95) | 0.952 | 1.8% | 69.6% | 61.0% | 0.959 | 10.0% | 全源が商用可 |
...
### 追加: 顔だけを独立に測り直した(615枚・学習に一度も使っていない写真から作成)

評価セットの `faces` は実写40枚しかなく、しかも2019年のStyleGANとFFHQという古い組み合わせなので、
**製品の実際の用途(いまのAI生成ポートレート vs 実在の人の写真)**を測るために独立のテストを作った。
実写=Pexels(選択用devに使った写真・学習には未使用)から顔を切り出した270枚、AI=xAI Aurora(評価4セットに出てこない生成器)から同じ手順で切り出した345枚。

| | 実在の人の顔の誤判定 | AI生成の顔の検出 | AUC |
|---|---|---|---|
| **v3.1(採用)** | **1.1%(3/270)** | 92.2% | **0.995** |
| v3(先行版) | 2.2%(6/270) | 92.5% | 0.990 |

つまり**いまのAIポートレートの判定は両版とも9割以上できており、実在の人を誤って「AI」と言う割合は1%程度**。
古いGAN風の切り取り(FFHQ様式)では誤判定が5%まで上がる、というのが正確な言い方になる。

### v3 → v3.1 で何を変えたか

v3 は非商用の FFHQ / CelebA-HQ を外した副作用で「顔の接写=AI」に傾いていた。そこで商用利用可の写真を
67,886枚追加収集し、そこから顔を7,352枚切り出して実写側に追加(AI側も同じ検出器・同じ余白で4,407枚)。
顔の領域の枚数比を「実写1.3 : AI1」に整えた(全体の比率の鏡像)。結果、同じ誤警報率(1.8%)で
検出率・2025年世代検出・顔の誤判定のすべてが v3 と同等以上になった。

### 正直に書く弱点

- **古いGAN風に切り取られた顔(FFHQ様式)では、実在の人を約5%「AI」と誤判定する**(v2は0%だった。ただしv2はStyleGAN顔を22.5%しか検出できず、顔の領域では実質何も言わないモデルだった)。原因は明確で、非商用の FFHQ / CelebA-HQ を学習から外したため、実写側の「顔の接写」が失われたこと。商用可の写真(Open Images / Unsplash)から顔を検出して切り出し、AI側からも同じ方法で切り出して両クラスに足す対策を打ったが、完全には戻っていない(v2は代わりにStyleGAN顔の検出が22.5%しかなく、顔の領域では実質「何も言わない」モデルだった。v3は50%検出する)
