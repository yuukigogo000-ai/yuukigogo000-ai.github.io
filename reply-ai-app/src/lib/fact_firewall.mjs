...
const HARD_PATTERNS = [
  ...
  ['past_experience', /(?:行っ|来|寄っ|訪れ|泊まっ|食べ|飲ん|読ん|観|見|買っ|作っ|通っ|回っ|開拓し|参加し|体験し|履い|着|使っ|住ん|やっ|覚え|習っ)(?:た(?!く[なて]|い)|て(?:き|い)?(?:た(?!く[なて]|い)|ます|る)|ちゃった|で(?:た(?!く[なて]|い)|る|ます|います))|(?:行き|来|寄り|訪れ|泊まり|食べ|飲み|読み|観|見|買い|作り|通い|回り|開拓し|参加し|体験し|履き|着|使い|住み|やり)ました(?!ら)/],
  // 習慣・頻度
...
export function findPersonalFacts(text, ctx = {}) {
  ...
  for (const clause of splitClauses(text)) {
    ...
    for (const [code, re] of HARD_PATTERNS) {
      const m = re.exec(c);
      if (!m) continue;
      if (isNegated(c, m.index + m[0].length)) continue;   // 否定形は安全(「行ったことない」)
      if (isGrounded(c, grounding)) continue;              // 有効な自分情報・自分の過去発言に根拠あり
      found.push({ code, level: 'hard_reject', clause: c.trim(), detail: `根拠のない個人事実(${code}): 「${m[0]}」` });
    }
