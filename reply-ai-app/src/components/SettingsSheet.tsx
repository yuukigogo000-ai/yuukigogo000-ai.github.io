...
export function SettingsSheet({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-40 bg-scrim/50" />
        <Dialog.Content
          aria-describedby={undefined}
          className="fixed inset-x-0 bottom-0 z-50 mx-auto max-h-[88vh] w-full max-w-[560px] overflow-y-auto rounded-t-md border border-line bg-surface pb-[max(20px,env(safe-area-inset-bottom))]"
        >
          <div className="sticky top-0 flex items-center justify-between border-b border-line bg-surface px-5 py-3.5">
            <Dialog.Title className="text-body font-bold">設定</Dialog.Title>
            <Dialog.Close
              id="sheetClose"
              aria-label="閉じる"
          ...
          <div className="space-y-6 px-5 pt-5">
            <section>
              ...
              />
              <label className="mt-3 flex items-center gap-2.5 text-sub">
                <input
                  type="checkbox"
                  id="rememberKey"
                  checked={remember}
            ...
            <section>
              <h3 className="text-sub font-bold text-ink-muted">このアプリの方針</h3>
              <ul className="mt-2 space-y-2 text-sub leading-relaxed">
                <li>
                  <b>別人を演じさせない。</b>
            ...
            <section className="border-t border-line pt-4 text-label leading-relaxed text-ink-muted">
              <p>
