...
## 最短手順(推奨・PC でやることは1行だけ)

VS Code のターミナルでこのリポジトリを開き、`claude` を起動して次を貼る:

```
handoff/PROMPT_FOR_LOCAL_CLAUDE.md を読んで、その指示を実行して
```

ローカルの Claude Code が、収集スクリプトの実行 → 空欄の記入 → 秘密の混入確認 → コミット → 同じブランチへ push まで行います。
終わったら `handoff/INVENTORY_2026-09.md` を ChatGPT にアップロードするだけです(ステップ 4)。
以下の5ステップは、手動でやる場合の内訳です。
