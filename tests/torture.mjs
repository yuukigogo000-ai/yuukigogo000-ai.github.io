...
await openMore(page); await page.click("#renameSetlistBtn");
...
console.log("--- D. 極端なデータ量(80曲) ---");
{
  ...
  await bigCtx.addInitScript(NO_WEB_SHARE);
  ...
  // 共有リンク(巨大)
  await openMore(bp); await bp.click("#shareBtn");
...
console.log("--- F. 本番モード中の削除 ---");
{
  ...
  await openMore(page); await page.click("#newSetlistBtn");
  await page.selectOption("#setlistSelect", { index: 0 }); // 元のセトリ
  await openMore(page); await page.click("#deleteSetlistBtn"); // confirm自動OK
  // さらにライブラリから次の曲を削除
  await page.click('[data-tab="library"]');
...
console.log("--- G. 空セトリ+通し練習中のUndo ---");
{
  ...
  await page.click('[data-tab="setlist"]');
  await openMore(page); await page.click("#printBtn");
  await page.waitForTimeout(200);
  await openMore(page); await page.click("#shareBtn");
