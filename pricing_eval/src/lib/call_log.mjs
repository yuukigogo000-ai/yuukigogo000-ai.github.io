...
export const CALL_LOG_PATH = 'pricing_eval/runs/call_log.jsonl';
const TERMINAL = new Set(['SUCCEEDED', 'FAILED']);

/** 呼び出し直前に呼ぶ。追記が完了してから network を叩くこと(戻り値の callId を callEnded に渡す) */
export function callStarted({ runId, caseId, modelId, attemptNo, worstCaseUsd, apiOperation = 'Converse' }, path = CALL_LOG_PATH) {
...
export function callEnded({ callId, status, costUsd = null, requestId = null, httpStatus = null, failureKind = null }, path = CALL_LOG_PATH) {
...
/** 壊れた行は読み飛ばす(数は返す) */
export function loadCallLog(path = CALL_LOG_PATH) {
 ...
 */
export function recordedSpendUsd(runsDir = 'pricing_eval/runs') {
